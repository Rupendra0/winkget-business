...
export default function VendorAddMenuItemForm({
  categories,
  lockedCategory,
  sellerName,
  mode = "create",
  ...
  const categoryOptions = useMemo<VendorCatalogSubcategory[]>(() => {
    ...
    // For restaurant vendors, we filter by locked category if present
  ...
  const validateForm = () => {
    ...
    if (!nameVal) {
      errors.productName = "Menu item name is required.";
    } else if (countWords(nameVal) > MAX_PRODUCT_NAME_WORDS) {
      errors.productName = `Menu item name cannot exceed ${MAX_PRODUCT_NAME_WORDS} words.`;
    }
    ...
    if (!fieldValues.mainImage && !existingMainImageUrl) {
      errors.mainImage = "A main food image is required.";
    }
  ...
  const submitProduct = async (event: FormEvent) => {
    ...
    try {
      ...
      const fallback = isEditMode
        ...
        : "Failed to publish menu item. Please try again.";
    ...
    <section className="rounded-[24px] border-2 border-[#d9ccb7] bg-[linear-gradient(180deg,#fffaf3,#fff5e9)] px-4 py-4 shadow-[0_16px_40px_rgba(87,63,38,0.08)] sm:px-5 sm:py-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            {isEditMode
              ? "Update existing dish details using the same form."
              : "Upload dish details with a clear main food image."}
          </p>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={submitProduct} className="mt-5 space-y-5">
            ...
            <section className="rounded-2xl border-2 border-[#d9ccb7] bg-[#fffdf8] p-4 shadow-[0_8px_18px_rgba(87,63,38,0.06)]">
              ...
              <div className="mt-3 grid gap-4 md:grid-cols-2">
                <label className="block text-sm text-slate-700">
                  Dish Name<span className="ml-1 text-rose-500">*</span>
                  <input
                    type="text"
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                          This image will be used as food item cover.
                        </p>
                ...
                {customHighlights.map((highlight, index) => (
                  <div key={`highlight-${index}`} className="relative flex items-center">
                    <input
                      ...
                      placeholder="e.g. Spicy, Veg, Gluten-free"
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {saving
              ? (isEditMode ? "Updating Menu Item..." : "Publishing Menu Item...")
              : (isEditMode ? "Update Menu Item" : "Publish Menu Item")}
          </button>
