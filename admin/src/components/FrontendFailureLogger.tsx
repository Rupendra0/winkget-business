...
export default function FrontendFailureLogger() {
  ...
  useEffect(() => {
    ...
    const patchedFetch: typeof window.fetch = async (input, init) => {
      ...
      try {
        ...
        if (!requestUrl.includes("/api/dev-logs") && !response.ok) {
          void logFailure("admin-frontend-fetch", `HTTP ${response.status} while requesting ${requestUrl || "unknown endpoint"}`, {
            method,
        ...
        if (!requestUrl.includes("/api/dev-logs")) {
          void logFailure("admin-frontend-fetch", toMessage(error), {
            method,
            url: requestUrl,
          });
        }
    ...
    const handleError = (event: ErrorEvent) => {
      void logFailure("admin-frontend-runtime", event.message || toMessage(event.error), {
        file: event.filename,
        line: event.lineno,
        column: event.colno,
    ...
    const handleRejection = (event: PromiseRejectionEvent) => {
      void logFailure("admin-frontend-promise", toMessage(event.reason), {
