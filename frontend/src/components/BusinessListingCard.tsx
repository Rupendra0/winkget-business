...
function BusinessListingCardComponent({
  ...
  return (
    <article
      ...
      className={`group rounded-2xl border border-gray-100 bg-white p-4 shadow-md transition-all duration-300 hover:scale-[1.02] hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${accent.focusRingClass} ${
        className || ""
      }`}
      ...
      <div className="mt-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <Link
            ...
            className="min-w-0 flex-1 truncate text-lg font-semibold text-gray-800 transition-colors hover:text-gray-900"
          >
            {displayName}
          </Link>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {highlights.map((tag) => (
            <span
              key={`${listing.id}-${tag}`}
              className={`inline-flex rounded-full px-3 py-1 text-xs font-medium ${accent.tagClass}`}
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-3 flex gap-2">
          {whatsappHref ? (
            <a
              ...
              rel="noreferrer"
              onClick={stopCardNavigation}
              className={`inline-flex w-full flex-1 items-center justify-center gap-1 rounded-xl py-2.5 text-sm font-medium text-white transition ${accent.primaryButtonClass}`}
            ...
            <button
              type="button"
              disabled
              className="inline-flex w-full flex-1 items-center justify-center rounded-xl border border-gray-200 bg-gray-100 py-2.5 text-sm font-medium text-gray-400"
          ...
          {callHref ? (
            <a
              href={callHref}
              onClick={stopCardNavigation}
              className={`inline-flex w-full flex-1 items-center justify-center gap-1 rounded-xl py-2.5 text-sm font-medium transition ${accent.secondaryButtonClass}`}
